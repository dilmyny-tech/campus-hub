...
def _seed_data(c):
    ...
    secondhand = [
        ...
        ("羽毛球拍双拍带包", "适合新手对打，送几个球。", 95, "运动"),
        ("跑步鞋42码穿过两次", "缓震不错，码数不合适出。", 150, "运动"),
        ("滑板初学者款", "练习平花够用，附护具。", 110, "运动"),
        ("考研专业课笔记打印版", "学长整理的重点笔记，已装订。", 22, "书籍"),
        ("自行车通勤代步", "入门款骑行正常，车锁送，宿舍楼下自提。", 260, "其他"),
        ("吉他入门款带琴包", "练习足够，附拨片和变调夹。", 200, "其他"),
        ("数位板绘画手绘板", "画画网课都能用，含压感笔。", 230, "数码"),
        ("保温杯大容量316钢", "保温效果好，全新未使用。", 38, "生活用品"),
        ("雅思真题剑桥4-18", "备考资料齐全，部分有笔记。", 70, "书籍"),
        ("宿舍懒人沙发", "躺着舒服，可拆洗，宿舍楼下自提。", 75, "生活用品"),
        ("蓝牙音箱便携防水", "户外宿舍都能用，音质不错。", 90, "数码"),
        ("二手iPhone 12 128G", "屏幕无划痕，电池健康85%，图书馆门口面交。", 1800, "数码"),
        ("桌面置物架宿舍用", "收纳书本零食，省桌面空间。", 25, "生活用品"),
        ("【待审核】二手吉他效果器", "刚提交，等待管理员审核。", 300, "数码"),
        ("【已驳回】疑似违规描述演示", "该条用于演示驳回状态。", 50, "其他"),
    ]

    # 跑腿需求（12 条）：(标题, 描述, 报酬, 分类)
    errands = [
        ("菜鸟驿站代取快递", "包裹较大较重，菜鸟驿站代取送到3号宿舍楼下，报酬可议。", 8, "快递代拿"),
        ("代取食堂外卖到信息楼", "午饭高峰外卖代拿，12点前送到信息楼B座。", 5, "外卖代取"),
        ("帮占图书馆南区座位", "图书馆占座，明早8点南区靠窗，占2小时。", 6, "其他跑腿"),
        ("顺丰代取到5号楼", "顺丰代取，两个中等包裹送到5栋宿舍楼下。", 7, "快递代拿"),
        ("帮带校门口奶茶两杯", "奶茶外卖代拿，少冰三分糖，送到5栋。", 6, "外卖代取"),
        ("打印并装订论文20页", "黑白打印加简单装订，明天上午要。", 10, "其他跑腿"),
        ("菜鸟驿站代取小件", "菜鸟驿站代取，取件码已给，送到东区宿舍楼下。", 6, "快递代拿"),
        ("帮带早餐到三教", "包子豆浆，8点前送到三教门口。", 5, "外卖代取"),
        ("教材借阅顺路归还", "3本书顺路还到总馆借阅处即可。", 5, "其他跑腿"),
        ("代取实验材料到二楼", "实验楼一楼领取送到二楼实验室。", 8, "其他跑腿"),
        ("顺丰代取到东区宿舍", "顺丰代取，一个小包裹送到东区宿舍楼下。", 7, "快递代拿"),
        ("【待审核】帮带宵夜到9栋", "炒粉一份，外卖代拿送到9栋楼下。", 8, "外卖代取"),
    ]

    # 发布时间分布（单位：天前）。平台已运营约 19 天：多数发布集中在 7~18 天前，
    # 最近 7 天为自然波动的零散新增（每天计数大致 2/0/3/1/0/2/1，含无人发帖的日子）。
    sh_days = [
        0, 8, 12, 7, 15, 9, 18, 11, 14, 8, 2, 16, 10, 13, 7, 17,
        9, 12, 8, 15, 11, 18, 9, 14, 7, 16, 10, 13, 8, 17, 11, 15,
        9, 12, 7, 2, 5,
    ]
    er_days = [9, 14, 6, 11, 3, 16, 8, 13, 5, 10, 2, 0]

    base = now.replace(hour=0, minute=0, second=0, microsecond=0)
    pin_sh = {5, 8, 20}  # 当前置顶中的二手商品索引（均为已通过）
    ...
    top_orders = [(("s", 5), 1), (("s", 8), 4), (("s", 20), 6), (("s", 2), 9), (("s", 11), 13)]
